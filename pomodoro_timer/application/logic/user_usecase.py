...
class UserUsecase:
    ...
    def native_login(self, code) -> User:
        token = Token(access_token=code, refresh_token="")
        gmail_address = self._google_repository.get_gmail_address(token)
        id = create_user_id(gmail_address)
        user_factory = Factory()
        exists_user = self._user_repository.is_exist(user_factory.create_user(id))
        if exists_user:
            ...
            user = user_factory.create_user(id, token)
            self._user_repository.register_user(user)
        return user
